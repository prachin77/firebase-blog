...
const Detail = ({ setActive, user }) => {
  ...
  const getBlogDetail = async () => {
    ...
    // Check if tags array is non-empty before querying related blogs
    const blogTags = blogDetail.data().tags || [];
    if (blogTags.length > 0) {
      const relatedBlogsQuery = query(
        blogRef,
        where("tags", "array-contains-any", blogTags, limit(3))
      );
      const relatedBlogSnapshot = await getDocs(relatedBlogsQuery);
      const relatedBlogs = relatedBlogSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setRelatedBlogs(relatedBlogs);
    } else {
      // If tags array is empty, set relatedBlogs to an empty array
      setRelatedBlogs([]);
    }
